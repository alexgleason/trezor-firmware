...
class RustLayout(ui.Layout):
    ...
    def page_count(self) -> int:
        ...
        # TODO: We could do it some other way
        # TT does this:
        # def page_count(self) -> int:
        #     return self.layout.page_count()

        self._place_layout()
        raw_content = self._read_content_raw()

        def kw_int(key: str) -> int | None:
            for key_index, item in enumerate(raw_content):
                if item == key:
                    if raw_content[key_index + 1] == "::":
                        return int(raw_content[key_index + 2])
            return None

        return kw_int("scrollbar_page_count") or kw_int("page_count") or 1
